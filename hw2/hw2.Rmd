...
```{r}
for (i in 1:1000){
  stopifnot(floor.log2(i)==floor(log(i, base=2)))
}
...
monte.carlo.pi = function(n, draw=FALSE) {
  x=runif(n,-1,1)
  y=runif(n,-1,1)
  result=apply(cbind(x,y),1,function(x) magnitude(x[1],x[2]))
  pihat=sum(result<=1)/n*4
  
  if (draw==TRUE){
    par(pty="s")
    plot(x[result>1],y[result>1],col="red")
    points(x[result<=1],y[result<=1],col="blue")
...
```{r}
a=replicate(n = 500,expr = monte.carlo.pi(100))
hist(a,probability = T,col=2,border=F,xlab='pihat', main='Comparision')
#b<-dnorm(a,mean=mean(a),sd=sd(a)) 
b=replicate(n = 500,expr = monte.carlo.pi(1000))
hist(b,probability = T,add=T,col=3)
lines(density(a),col=2)
lines(density(b),col=3)
legend(x=3.3,y=2.5,legend = c("100","500"),col = c("red","green"),lwd=c(1,1))
...
```{r}
scores$Pct.gr=sapply(scores$Pct,PercentageGrade)
scores$Pctile.gr=sapply(scores$Pctile,PercentileGrade)
scores$Grade=mapply(x=scores$Pct.gr,y=scores$Pctile.gr,FUN = CourseGrade,USE.NAMES = FALSE)
scores
```
...
```{r}
round(tapply(scores$E1,scores$Grade,mean),1)
...
```{r}
apply(scores[c("Q1","Q2","Q3","Q4")],2,mean)
...
```{r}
apply(scores[c("Q1","Q2","Q3","Q4")],1,mean)
```
